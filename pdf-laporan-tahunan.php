...
$html = '<table align="center"><tr><td><span style="font-weight:bold">LAPORAN TAHUNAN</span></td></tr></table><br/><br/>';
...
<tr style="background-color:#f0f8ff"><td width="100px">TAHUN</td><td>VISITATION</td><td>TRAINING</td><td>MEETING</td><td>TOTAL</td></tr>';

$query = "select YEAR(kunjungan.tanggal_kunjungan) as tahun,sum(IF(`id_keperluan`=1,jumlah_pengunjung,NULL)) Visit, sum(IF(`id_keperluan`=2,jumlah_pengunjung,NULL)) Train, sum(IF(`id_keperluan`=3,jumlah_pengunjung,NULL)) Meet, sum(jumlah_pengunjung) Total from kunjungan group by YEAR(tanggal_kunjungan) ASC";
...
$query1 = "SELECT DATE_FORMAT(kunjungan.tanggal_kunjungan, '%Y') as Tahun, sum(kunjungan.id_keperluan=1) AS Visitation, Sum(kunjungan.id_keperluan=2) AS Training, Sum(kunjungan.id_keperluan=3) AS Meeting, Count(kunjungan.id_keperluan) AS Total
